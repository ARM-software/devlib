...
from devlib.utils.misc import (memoized, isiterable, convert_new_lines,
                               ...
                               _BoundTLSProperty, nullcontext, safe_extract)
...
from typing import (List, Set, Dict, Union, Optional, Callable, TypeVar,
                    Any, cast, TYPE_CHECKING, AsyncGenerator, Type, Pattern,
...
FSTAB_ENTRY_REGEX: Pattern[str] = re.compile(r'(\S+) on (.+) type (\S+) \((\S+)\)')
ANDROID_SCREEN_STATE_REGEX: Pattern[str] = re.compile('(?:mPowerState|mScreenOn|mWakefulness|Display Power: state)=([0-9]+|true|false|ON|OFF|DOZE|Dozing|Asleep|Awake)',
                                                      re.IGNORECASE)
ANDROID_SCREEN_RESOLUTION_REGEX: Pattern[str] = re.compile(r'cur=(?P<width>\d+)x(?P<height>\d+)')
ANDROID_SCREEN_ROTATION_REGEX: Pattern[str] = re.compile(r'orientation=(?P<rotation>[0-3])')
DEFAULT_SHELL_PROMPT: Pattern[str] = re.compile(r'^.*(shell|root|juno)@?.*:[/~]\S* *[#$] ',
                                                re.MULTILINE)
KVERSION_REGEX: Pattern[str] = re.compile(
...
GOOGLE_DNS_SERVER_ADDRESS: str = '8.8.8.8'
...
class Target(object):
    ...
    :type connection_settings: UserConnectionSettings, optional
    ...
    :type executables_directory: str, optional
    ...
    def __init__(self,
        ...
        self.shell_prompt: Pattern[str] = bytes_regex(shell_prompt)
        ...
        self._file_transfer_cache: Optional[str] = None
        self._max_async = max_async
        self.busybox: Optional[str] = None

# FIXME - find annotation for spec
    ...
    @property
    def abi(self) -> Optional[str]:  # pylint: disable=no-self-use
        """
        The primary application binary interface (ABI) of this target.

        :return: ABI name (e.g. "armeabi-v7a"), or None if unknown.
        :rtype: str or None
        """
        return None

    @property
    def supported_abi(self) -> List[Optional[str]]:
        """
    ...
    def _async_manager(self) -> 'AsyncManager':
        ...
        :rtype: devlib.utils.asyn.AsyncManager
        """
    ...
    async def connect(self, timeout: Optional[int] = None,
        ...
        :type check_boot_completed: bool, optional
        :param max_async: The number of parallel async connections to allow.
        :type max_async: int, optional
        ...
        self._resolve_paths()
        # FIXME - should we raise exception if self.working_directory, executables_directory or abi is None?
    ...
    def disconnect(self) -> None:
        ...
        """
        with self._lock:
            thread_conns: Set[SupportedConnections] = self._conn.get_all_values()
    ...
    async def setup(self, executables: Optional[List[str]] = None) -> None:
        ...
        :type executables: list(str), optional
        """
    ...
    async def _prepare_xfer(self, action: str, sources: List[str], dest: str,
        ...
        def target_paths_kind(paths: List[str], as_root: bool = False) -> List[Optional[str]]:
            ...
            if _paths:
                ...
                res: str = self.execute(cmd, as_root=as_root)
        ...
        def host_paths_kind(paths: List[str], as_root: bool = False) -> List[Optional[str]]:
        ...
        def dst_mkdir(path: Optional[str]) -> None:
            if path:
                _dst_mkdir(path)

        def rewrite_dst(src: str, dst: str) -> str:
    ...
    async def push(self, source: str, dest: str, as_root: bool = False,
        ...
        mapping: Dict[Tuple[str, ...], str] = await self._prepare_xfer.asyn('push', sources, dest, pattern=source if globbing else None, as_root=as_root)

        def do_push(sources: Tuple[str, ...], dest: str) -> None:
            for src in sources:
            ...
            for sources_map, dest_map in mapping.items():
                for source in sources_map:
                    async with self._xfer_cache_path(source) as device_tempfile:
                        do_push(tuple([source]), device_tempfile)
                        await self.execute.asyn("mv -f -- {} {}".format(quote(device_tempfile), quote(dest_map)), as_root=True)
        else:
            for sources_map, dest_map in mapping.items():
    ...
    async def pull(self, source: str, dest: str, as_root: bool = False,
        ...
        mapping: Dict[Tuple[str, ...], str] = await self._prepare_xfer.asyn('pull', sources, dest, pattern=source if globbing else None, as_root=as_root)

        def do_pull(sources: Tuple[str, ...], dest: str) -> None:
            for src in sources:
                self.async_manager.track_access(
            ...
            for sources_map, dest_map in mapping.items():
                for source in sources_map:
                    async with self._xfer_cache_path(source) as device_tempfile:
                        ...
                        do_pull(tuple([device_tempfile]), dest_map)
        else:
            for sources_map, dest_map in mapping.items():
    ...
    def _prepare_cmd(self, command: SubprocessCommand, force_locale: str) -> SubprocessCommand:
        ...
        if force_locale:
            ...
            command = 'export LC_ALL={} && {}'.format(quote(force_locale), cast(str, command))

        # Ensure to use deployed command when availables
        if self.executables_directory:
            command = "export PATH={}:$PATH && {}".format(quote(self.executables_directory), cast(str, command))
    ...
    def _execute(self, command: SubprocessCommand, timeout: Optional[int] = None, check_exit_code: bool = True,
        ...
        :type command: str or SubprocessCommand
    ...
    def background_invoke(self, binary: str, args: Optional[Union[str, Iterable[str]]] = None, in_directory: Optional[str] = None,
        ...
        :param args: arguments to be passed to the binary. The can be either a list or
               a string.
        :type args: str or list of str, optional
        :param in_directory:  execute the binary in the  specified directory. This must
                        be an absolute path.
        :type in_directory: str, optional
        :param on_cpus:  taskset the binary to these CPUs. This may be a single ``int`` (in which
                   ...
                   will be interpreted as a comma-separated list of cpu ranges, e.g.
                   ``"0,4-7"``.
        :type on_cpus: int or list(int) or str, optional
        :param as_root: Specify whether the command should be run as root
        :type as_root: bool
...
'''
    ...
    @asyn.asynccontextmanager
    async def make_temp(self, is_directory: Optional[bool] = True, directory: Optional[str] = '',
                        prefix: Optional[str] = 'devlib-test') -> AsyncGenerator:
        """
        ...
        working_directory = directory or self.working_directory
        if prefix:
            _prefix = prefix
        else:
            _prefix = ''
        temp_obj: Optional[str] = None
        try:
            if working_directory:
                cmd = f'mktemp -p {quote(working_directory)} {quote(_prefix)}-XXXXXX'
                if is_directory:
                    cmd += ' -d'
    ...
    async def list_file_systems(self) -> List['FstabEntry']:
        ...
        for line in output.split('\n'):
            ...
            if not line:
                continue
            match: Optional[Match[str]] = FSTAB_ENTRY_REGEX.search(line)
            if match:
    ...
    def get_workpath(self, name: str) -> Optional[str]:
        """
    ...
    @asyn.asyncf
    async def tempfile(self, prefix: Optional[str] = '', suffix: Optional[str] = '') -> Optional[str]:
        """
    ...
    @asyn.asyncf
    async def list_online_cpus(self, core: Optional[str] = None) -> Optional[List[int]]:
        """
    ...
    async def get_installed(self, name: str, search_system_binaries: bool = True) -> Optional[str]:
        ...
        :type search_system_binaries: bool
    ...
    @asyn.asyncf
    async def _setup_shutils(self) -> None:
        """
        ...
        """
        shutils_ifile: str = os.path.join(PACKAGE_BIN_DIRECTORY, 'scripts', 'shutils.in')
    ...
    def _install_module(self, mod: Union[str, Type[Module]],
        ...
        if name:
            if params is None or self._modules.get(name, {}) is None:
                ...
                try:
                    return module.install(self, **params)
                except Exception as e:
                    if log:
...
class LinuxTarget(Target):
    ...
    async def capture_screen(self, filepath: str) -> None:
        ...
        try:
            ...
            tmpfile: str = await self.tempfile.asyn()
    ...
    def _resolve_paths(self) -> None:
        ...
        if self.working_directory is None:
            self.working_directory: str = self.path.join(self.execute("pwd").strip(), 'devlib-target')
        self._file_transfer_cache: str = self.path.join(self.working_directory, '.file-cache')
        if self.executables_directory is None:
            self.executables_directory: str = self.path.join(self.working_directory, 'bin')
...
class AndroidTarget(Target):
    ...
    :type connection_settings: AdbUserConnectionSettings or None
    ...
    def supported_abi(self) -> List[Optional[str]]:
        ...
        mapped_result: List[Optional[str]] = []
    ...
    def screen_resolution(self) -> Tuple[int, int]:
        ...
        match: Optional[Match[str]] = ANDROID_SCREEN_RESOLUTION_REGEX.search(output)
        if match:
    ...
    async def connect(self, timeout: Optional[int] = 30,
        ...
        :type check_boot_completed: bool
    ...
    async def capture_screen(self, filepath: str) -> None:
        ...
        """
        if self.path and self.working_directory:
    ...
    async def getprop(self, prop: Optional[str] = None) -> Optional[Union[str, AndroidProperties]]:
        ...
        :rtype: AndroidProperties or str
        """
    ...
    async def get_package_info(self, package: str) -> Optional['installed_package_info']:
        ...
        :rtype: installed_package_info or None
        """
    ...
    async def refresh_files(self, file_list: List[str]) -> None:
        ...
        """
        if self.path and (self.is_rooted or (await self.get_sdk_version.asyn()) < 24):  # MM and below
    ...
    async def install_executable(self, filepath: str, with_name: Optional[str] = None,
        ...
        if self.path:
            on_device_file: str = self.path.join(self.working_directory, executable_name)
            on_device_executable: str = self.path.join(self.executables_directory, executable_name)
            ...
            raise TargetStableError('path is not assigned')
    ...
    async def dump_logcat(self, filepath: str, filter: Optional[str] = None,
        ...
        :type logcat_format: str, optional
    ...
    async def is_screen_on(self) -> bool:
        ...
        match: Optional[Match[str]] = ANDROID_SCREEN_STATE_REGEX.search(output)
        if match:
    ...
    def _resolve_paths(self) -> None:
        """
        Finalize the paths for working directory, executables directory, etc.
        If not user-defined, default them to a known location on the device.
        """
        if (self.working_directory is None) and self.path:
            self.working_directory: str = self.path.join(self.external_storage, 'devlib-target')
        if self.path:
            self._file_transfer_cache: str = self.path.join(self.working_directory, '.file-cache')
        if self.executables_directory is None:
            self.executables_directory: str = '/data/local/tmp/bin'
...
class KernelVersion(object):
    ...
    def __init__(self, version_string: str):
        ...
        match: Optional[Match[str]] = KVERSION_REGEX.match(version_string)
        if match:
...
class TypedKernelConfig(Mapping):   # type: ignore
    ...
    def _parse_val(k: str, v: Union[str, int, HexInt,
        ...
        :type v: str or int or KernelConfigTristate
...
class LocalLinuxTarget(LinuxTarget):
    ...
    :type conn_cls: Type[LocalConnection]
    ...
    def _resolve_paths(self) -> None:
        ...
        """
        if self.working_directory is None:
            self.working_directory: str = '/tmp/devlib-target'
        self._file_transfer_cache: str = self.path.join(self.working_directory, '.file-cache')
        if self.executables_directory is None:
            self.executables_directory: str = '/tmp/devlib-target/bin'
...
class ChromeOsTarget(LinuxTarget):
    ...
    def _resolve_paths(self) -> None:
        ...
        """
        if self.working_directory is None:
            self.working_directory: str = '/mnt/stateful_partition/devlib-target'
        self._file_transfer_cache: str = self.path.join(self.working_directory, '.file-cache')
        if self.executables_directory is None:
            self.executables_directory: str = self.path.join(self.working_directory, 'bin')
