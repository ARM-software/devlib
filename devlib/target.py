...
class Target(object):
    ...
    async def connect(self, timeout: Optional[int] = None,
        ...
        self.busybox = self.install(os.path.join(PACKAGE_BIN_DIRECTORY, self.abi or '', 'busybox'), timeout=30)
        ...
        if self.tmp_directory is None:
            ...
            try:
                ...
                tmp = self.path.join(self.working_directory, 'tmp') if self.path else ''
            else:
                tmp = tmp.strip()
...
'''
    ...
    async def tempfile(self, prefix: Optional[str] = None, suffix: Optional[str] = None) -> Optional[str]:
        ...
        path = self.path.join(self.tmp_directory, name) if self.path else ''
...
class AndroidTarget(Target):
    ...
    def _resolve_paths(self):
        if self.working_directory is None:
            self.working_directory = self.path.join(self.external_storage, 'devlib-target') if self.path else ''
        if self.tmp_directory is None:
            ...
            self.tmp_directory = tmp
        if self.executables_directory is None:
            self.executables_directory = self.path.join(self.tmp_directory, 'bin') if self.path else ''
