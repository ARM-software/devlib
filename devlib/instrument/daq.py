...
class DaqInstrument(Instrument):
    ...
    def __init__(self, target: 'Target', resistor_values: List[Union[int, str]],  # pylint: disable=R0914
        ...
        try:
            devices: List[str] = cast(daqpower.server.DaqServer, self.daq_client).list_devices()
        ...
        if time_as_clock_boottime:
            host_path: str = os.path.join(PACKAGE_BIN_DIRECTORY, self.target.abi or '',
                                          'get_clock_boottime')
    ...
    def reset(self, sites: Optional[List[str]] = None,
        ...
        cast(daqpower.server.DaqServer, self.daq_client).close()
        cast(daqpower.server.DaqServer, self.daq_client).configure(self.device_config)
    ...
    def start(self) -> None:
        ...
        cast(daqpower.server.DaqServer, self.daq_client).start()
    ...
    def stop(self) -> None:
        cast(daqpower.server.DaqServer, self.daq_client).stop()
