...
if TYPE_CHECKING:
    from devlib.target import Target, Node
...
class CpuidleState(object):
    ...
    def __init__(self, target: 'Target', index: int, path: str, name: str,
        ...
        self.id: str = self.target.path.basename(self.path) if self.target.path else ''
        self.cpu: str = self.target.path.basename(self.target.path.dirname(path)) if self.target.path else ''
    ...
    async def get(self, prop: str) -> str:
        ...
        get the property
        """
        property_path = self.target.path.join(self.path, prop) if self.target.path else ''
...
class Cpuidle(Module):
            ...
            cpu_name: sorted(
                (
                    CpuidleState(
                        ...
                        path=self.target.path.join(basepath, cpu_name, 'cpuidle', state_name) if self.target.path else '',
    ...
    @asyn.asyncf
    async def get_driver(self) -> Optional[str]:
        """
    ...
    def list_governors(self) -> List[str]:
        ...
        sysfile: str = self.target.path.join(self.root_path, 'available_governors') if self.target.path else ''
    ...
    async def get_governor(self) -> str:
        ...
        path = self.target.path.join(self.root_path, 'current_governor_ro') if self.target.path else ''
        if not await self.target.file_exists.asyn(path):
            path = self.target.path.join(self.root_path, 'current_governor') if self.target.path else ''
    ...
    def set_governor(self, governor: str) -> None:
        ...
        sysfile: str = self.target.path.join(self.root_path, 'current_governor') if self.target.path else ''
