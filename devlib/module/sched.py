...
from typing import (TYPE_CHECKING, cast, Match, Dict,
                    Any, List, Pattern, Union, Optional,
                    Tuple, Set)
if TYPE_CHECKING:
...
class SchedProcFSNode(object):
    ...
    _re_procfs_node: Pattern[str] = re.compile(r"(?P<name>.*\D)(?P<digits>\d+)$")
...
class SchedProcFSData(SchedProcFSNode):
    ...
    def available(target: 'Target') -> bool:
        ...
        for cpu in cpus:
            if target.file_exists(target.path.join(path, cpu, "domain0", "flags") if target.path else ''):
                return True
...
class SchedModule(Module):
    ...
    def probe(target: 'Target') -> bool:
        logger: logging.Logger = logging.getLogger(SchedModule.name)
    ...
    def cpu_dmips_capacity_path(cls, target: 'Target', cpu: int):
        ...
        return target.path.join(
            cls.cpu_sysfs_root,
            'cpu{}/cpu_capacity'.format(cpu)) if target.path else ''
