...
def _get_cgroup_controllers(target: LinuxTarget) -> ControllerDict:
    ...
    PROC_MOUNT_REGEX: Pattern[str] = re.compile(
...
@contextmanager
def _request_delegation(target: LinuxTarget) -> Generator[int]:
    """
...
@contextmanager
def _mount_v2_controllers(target: LinuxTarget) -> Generator[str]:
    """
...
@contextmanager
def _mount_v1_controllers(target: LinuxTarget, controllers: Set[str]) -> Generator[Dict[str, str]]:
    """
    ...
    # its mount path.
    @contextmanager
    def _mount_controller(controller: str) -> Generator[str]:
...
class _CGroupBase(ABC):
    ...
    def _get_pid_from_tid(self, tid: int) -> int:
        ...
        for line in status:
            ...
            match: Optional[Match] = re.match(r"\s*Tgid:\s*(\d+)\s*", line)
            if match:
...
class _CGroupV2Root(_CGroupV2):
    ...
    def _get_delegated_sub_path(cls, delegated_pid: int, target: LinuxTarget) -> Optional[str]:
        ...
        REL_PATH_REGEX: Pattern[str] = re.compile(r"0::\/(?P<path>.+)")

        for mount_path in relative_delegated_mount_paths:
            m: Optional[Match[str]] = REL_PATH_REGEX.match(mount_path)
            if m:
                return m.group("path")
            else:
                raise TargetStableError(
    ...
    @classmethod
    @contextmanager
    def _mount_filesystem(cls, target: LinuxTarget, requested_controllers: Set[str]) -> Generator[str]:
        """
...
class _CGroupV1Root(_CGroupV1):
    ...
    def _get_delegated_paths(
        ...
        REL_PATH_REGEX: Pattern[str] = re.compile(
        ...
        for mount_path in delegated_mount_paths:
            regex_match: Optional[Match[str]] = REL_PATH_REGEX.match(mount_path)
            if regex_match:
    ...
    @classmethod
    @contextmanager
    def _mount_filesystem(cls, target: LinuxTarget, requested_controllers: Set[str]) -> Generator[Dict[str, str]]:
        """
...
class RequestTree(_TreeBase):
    ...
    @contextmanager
    def setup_hierarchy(self, version: int, target: LinuxTarget) -> Generator['ResponseTree']:
        """
