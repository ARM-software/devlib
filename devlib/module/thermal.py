...
from typing import (TYPE_CHECKING, Dict, Match, Optional,
                    Tuple, List)
if TYPE_CHECKING:
...
class TripPoint(object):
    ...
    async def get_temperature(self) -> int:
        ...
        temp_file: str = self.target.path.join(self.zone.path, self.temp_node) if self.target.path else ''
    ...
    async def get_type(self) -> str:
        ...
        type_file: str = self.target.path.join(self.zone.path, self.type_node) if self.target.path else ''
...
class ThermalZone(object):
    ...
    def __init__(self, target: 'Target', root: str, _id: str):
        ...
        self.path = target.path.join(root, self.name) if target.path else ''
        self.trip_points: Dict[int, TripPoint] = {}
        self.type: str = self.target.read_value(self.target.path.join(self.path, 'type') if self.target.path else '')

        for entry in self.target.list_directory(self.path, as_root=target.is_rooted):
            re_match: Optional[Match[str]] = re.match('^trip_point_([0-9]+)_temp', entry)
    ...
    async def is_enabled(self) -> bool:
        ...
        value: str = await self.target.read_value.asyn(self.target.path.join(self.path, 'mode') if self.target.path else '')
    ...
    async def set_enabled(self, enabled: bool = True) -> None:
        ...
        await self.target.write_value.asyn(self.target.path.join(self.path, 'mode') if self.target.path else '', value)

    @asyn.asyncf
    async def get_temperature(self) -> int:
        """Returns the temperature of the thermal zone"""
        sysfs_temperature_file = self.target.path.join(self.path, 'temp') if self.target.path else ''
    ...
    async def get_policy(self) -> str:
        ...
        temp_file = self.target.path.join(self.path, 'policy') if self.target.path else ''
    ...
    async def set_policy(self, policy: str) -> None:
        ...
        :type policy: str
        """
        await self.target.write_value.asyn(self.target.path.join(self.path, 'policy') if self.target.path else '', policy)

    @asyn.asyncf
    async def get_offset(self) -> int:
        """Returns the temperature offset of the thermal zone"""
        offset_file: str = self.target.path.join(self.path, 'offset') if self.target.path else ''
    ...
    async def set_offset(self, offset: int) -> None:
        ...
        :type offset: int
        """
        await self.target.write_value.asyn(self.target.path.join(self.path, 'offset') if self.target.path else '', offset)
    ...
    async def set_emul_temp(self, offset: int) -> None:
        ...
        :type offset: int
        """
        await self.target.write_value.asyn(self.target.path.join(self.path, 'emul_temp') if self.target.path else '', offset)

    @asyn.asyncf
    async def get_available_policies(self) -> str:
        """Returns the policies available for the thermal zone"""
        temp_file: str = self.target.path.join(self.path, 'available_policies') if self.target.path else ''
        return await self.target.read_value.asyn(temp_file)
...
class ThermalModule(Module):
    ...
    def __init__(self, target: 'Target'):
        ...
        for entry in target.list_directory(self.thermal_root):
            re_match: Optional[Match[str]] = re.match('^(thermal_zone|cooling_device)([0-9]+)', entry)
            if not re_match:
