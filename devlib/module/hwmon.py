...
class HwmonSensor(object):
    ...
    def get_file(self, item: str) -> str:
        ...
        return self.target.path.join(self.path, filename) if self.target.path else ''
...
class HwmonDevice(object):
    ...
    def __init__(self, target: 'Target', path: str, name: str, fields: List[str]):
        ...
        if self.target.path:
            ...
            for entry in fields:
                match: Optional[Match[str]] = HWMON_FILE_REGEX.search(entry)
                if match:
...
class HwmonModule(Module):
    ...
    def scan(self) -> None:
        ...
        for entry_id, fields in values_tree.items():
            path: str = self.target.path.join(self.root, entry_id) if self.target.path else ''
