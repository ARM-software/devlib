...
class Controller(object):
    ...
    def __init__(self, kind: str, hid: int, clist: List[str]):
        ...
        self.logger: logging.Logger = logging.getLogger('CGroup.' + self.kind)
...
class CGroup(object):
    ...
    def __init__(self, controller: 'Controller', name: str, create: bool = True):
        self.logger: logging.Logger = logging.getLogger('cgroups.' + controller.kind)
    ...
    def set(self, **attrs: Union[str, List[int], int]) -> None:
        ...
        for idx in attrs:
            ...
            path: str = self.target.path.join(self.directory, attr_name) if self.target and self.target.path else ''
...
class CgroupsModule(Module):
    ...
    def __init__(self, target: 'Target'):
        ...
        self.logger: logging.Logger = logging.getLogger('CGroups')

        # Set Devlib's CGroups mount point
        self.cgroup_root: str = target.path.join(
            target.working_directory, 'cgroups') if target.path else ''
