...
from typing import (Optional, TYPE_CHECKING, Tuple, cast,
                    Callable, Union, IO, List, Sized, Dict,
                    Pattern, Generator, Type, Any)
...
logger: 'Logger' = logging.getLogger('ssh')
gem5_logger: 'Logger' = logging.getLogger('gem5-connection')
...
def _handle_paramiko_exceptions(command: Optional['SubprocessCommand'] = None) -> Generator:
    ...
    :type command: SubprocessCommand or None
...
def _resolve_known_hosts(strict_host_check: Optional[Union[bool, str, os.PathLike]]) -> str:
    ...
    :type strict_host_check: bool or str or os.PathLike
    ...
    if strict_host_check:
        if isinstance(strict_host_check, (str, os.PathLike)):
            ...
            path = Path(os.path.expandvars('~/.ssh/known_hosts'))
    else:
        path = Path('/dev/null')
...
def telnet_get_shell(host: str,
    ...
    :type original_prompt: str or None
...
class SshConnection(SshConnectionBase):
    ...
    def push(self, sources: Tuple[str, ...], dest: str, timeout: Optional[int] = None) -> None:
        """
        Transfer (push) one or more files from the host to the remote target.

        :param sources: A tuple of paths on the host system to be pushed.
    ...
    def pull(self, sources: Tuple[str, ...], dest: str, timeout: Optional[int] = None) -> None:
        """
    ...
    def _push_pull(self, action: Union[Literal['push'], Literal['pull']],
                   sources: Tuple[str, ...], dest: str, timeout: Optional[int]) -> None:
        """
    ...
    def background(self, command: 'SubprocessCommand', stdout: int = subprocess.PIPE,
        ...
        :rtype: ParamikoBackgroundCommand
    ...
    def _background(self, command: 'SubprocessCommand', stdout: int,
        ...
        command = "printf '%s\n' $$; exec sh -c {}".format(quote(cast(str, command)))
        channel: Optional['Channel'] = self._make_channel()
        if channel is None:
            raise TargetStableError("channel is None")

        def executor(cmd: str, timeout: Optional[float]) -> ChannelFiles:
            """
            executor to run the command via the paramiko channel
            """
            if channel is None:
                raise TargetStableError("channel is None")
        ...
        pidstr: Optional[str] = stdout_in.readline()
        if not pidstr:
            stderr_t = stderr_in.read()
            if channel and channel.exit_status_ready():
                ret: int = channel.recv_exit_status()
            else:
                ret = 126
        ...
        def create_out_stream(stream_in: int, stream_out: Union[int, bytes]
                              ) -> OutStreamType:
            """
            ...
            if stream_out == subprocess.DEVNULL:
                r: Union[Optional['BufferedReader'], int] = None
                w: Union[Optional['BufferedWriter'], int, bytes] = None
            ...
            # it
                ...
                r_tmp, w_tmp = os.pipe()
                r = os.fdopen(r_tmp, 'rb')
                w = os.fdopen(w_tmp, 'wb')
        ...
        out_streams: Dict[str, OutStreamType] = {
            name: create_out_stream(cast(int, stream_in), stream_out)
        ...
        def redirect_thread_f(stdout_in: 'ChannelFile', stderr_in: 'ChannelStderrFile',
                              out_streams: Dict[str, OutStreamType],
                              select_timeout: int) -> None:
            """
            the thread that does the background read/write operation
            """
            def callback(out_streams: Dict[str, OutStreamType], name: str,
                         chunk: bytes) -> Dict[str, OutStreamType]:
                try:
                ...
                try:
                    cast(BufferedWriter, w).write(chunk)
            ...
            try:
                _read_paramiko_streams(stdout_in, stderr_in, select_timeout, callback, copy.copy(cast(List[bytes], out_streams)))
        ...
        return ParamikoBackgroundCommand(
            ...
            stdout=cast(IO, out_streams['stdout'][0]),
            stderr=cast(IO, out_streams['stderr'][0]),
...
class TelnetConnection(SshConnectionBase):
    ...
    :type password_prompt: str, optional
    ...
    :type original_prompt: str, optional
