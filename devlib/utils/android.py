...
from devlib.connection import (ConnectionBase, AdbBackgroundCommand, PopenBackgroundCommand,
...
class AdbConnection(ConnectionBase):
    ...
    def push(self, sources: Tuple[str, ...], dest: str,
    ...
    def pull(self, sources: Tuple[str, ...], dest: str,
             timeout: Optional[int] = None) -> None:
        """
    ...
    def _push_pull(self, action: Union[Literal['push'], Literal['pull']],
        ...
        if timeout:
            ...
            bg_cmd: PopenBackgroundCommand = adb_command_background(
                device=self.device,
                conn=self,
                command=command,
            ...
            if isinstance(self.transfer_manager, TransferManager):
...
def adb_command_background(device: Optional[str], conn: AdbConnection, command: str,
                           adb_server: Optional[str] = None, adb_port: Optional[int] = None) -> PopenBackgroundCommand:
    """
    Build and run an 'adb' command in the background, returning a handle.

    :param device: The device serial or IP, or None if unspecified.
    :type device: str or None
    :param conn: The active AdbConnection instance.
    :type conn: AdbConnection
    :param command: The adb subcommand string.
    :type command: str
    :param adb_server: Custom adb server address if required.
    :type adb_server: str or None
    :param adb_port: Custom adb server port if required.
    :type adb_port: int or None
    :returns: A PopenBackgroundCommand object referencing the running 'adb' command.
    :rtype: PopenBackgroundCommand
    """
    full_command: str = get_adb_command(device, command, adb_server, adb_port)
    logger.debug(full_command)
    popen: 'Popen' = get_subprocess(full_command, shell=True)
