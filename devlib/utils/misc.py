...
from typing import (Union, List, Optional, Tuple, Set,
                    Any, Callable, Dict, Generator, TYPE_CHECKING,
                    Type, cast, Pattern)
...
check_output_logger: 'Logger' = logging.getLogger('check_output')
...
def check_subprocess_output(
    ...
    :type ignore: int, list of int, "all", or None
    :param inputtext: Optional text or bytes to send to the process's stdin.
    :type inputtext: str, bytes, or None
...
class ExtendedHostError(HostError):
    ...
    :type orig_exc: Exception or None
    """
...
def redirect_streams(stdout: int, stderr: int,
    ...
    :rtype: (int, int, SubprocessCommand)
    """
...
_bash_color_regex: Pattern[str] = re.compile(r'\x1b\[[0-9;]*[a-zA-Z]')
...
class tls_property:
    ...
    def __get__(self, instance: 'Target', owner: Optional[Type['Target']] = None) -> '_BoundTLSProperty':
        ...
        :type owner: Type[Target] or None
        :returns: A bound TLS property proxy.
        :rtype: _BoundTLSProperty
        """
...
class _BoundTLSProperty:
    ...
    :type tls_property: tls_property
