...
class FtraceCollector(CollectorBase):
    ...
    def __init__(self, target: 'Target',
        ...
        # Setup tracing paths
        self.available_events_file    = self.target.path.join(self.tracing_path, 'available_events')  # type: ignore
        self.available_functions_file = self.target.path.join(self.tracing_path, 'available_filter_functions')  # type: ignore
        self.current_tracer_file      = self.target.path.join(self.tracing_path, 'current_tracer')  # type: ignore
        self.function_profile_file    = self.target.path.join(self.tracing_path, 'function_profile_enabled')  # type: ignore
        self.marker_file              = self.target.path.join(self.tracing_path, 'trace_marker')  # type: ignore
        self.ftrace_filter_file       = self.target.path.join(self.tracing_path, 'set_ftrace_filter')  # type: ignore
        self.available_tracers_file   = self.target.path.join(self.tracing_path, 'available_tracers')  # type: ignore
        self.kprobe_events_file       = self.target.path.join(self.tracing_path, 'kprobe_events')  # type: ignore
        ...
        if not no_install:
            host_file = os.path.join(PACKAGE_BIN_DIRECTORY, self.target.abi, 'trace-cmd')  # type: ignore
