...
from typing import (Pattern, Optional, Match, Tuple, List,
                    Union, Any, Generator, TYPE_CHECKING)
if TYPE_CHECKING:
    from devlib.target import Target

_LOGGER: logging.Logger = logging.getLogger('dmesg')
...
class KernelLogEntry(object):
    ...
    _TIMESTAMP_MSG_REGEX: Pattern[str] = re.compile(r'\[(.*?)\] (.*)')
    _RAW_LEVEL_REGEX: Pattern[str] = re.compile(r'<([0-9]+)>(.*)')
    _PRETTY_LEVEL_REGEX: Pattern[str] = re.compile(r'\s*([a-z]+)\s*:([a-z]+)\s*:\s*(.*)')
    ...
    def from_str(cls, line: str, line_nr: int = 0) -> 'KernelLogEntry':
        ...
        :rtype: KernelLogEntry

        """

        def parse_raw_level(line: str) -> Tuple[str, Union[str, Any]]:
            match: Optional[Match[str]] = cls._RAW_LEVEL_REGEX.match(line)
            if not match:
        ...
        def parse_pretty_level(line: str) -> Tuple[str, str, str]:
            match: Optional[Match[str]] = cls._PRETTY_LEVEL_REGEX.match(line)
            if not match:
        ...
        def parse_timestamp_msg(line: str) -> Tuple[timedelta, str]:
            match: Optional[Match[str]] = cls._TIMESTAMP_MSG_REGEX.match(line)
            if not match:
    ...
    @classmethod
    def from_dmesg_output(cls, dmesg_out: Optional[str], error: Optional[str] = None) -> Generator['KernelLogEntry', Any, None]:
        """
        ...
        :rtype: Generator[KernelLogEntry, Any, None]

        .. note:: The same restrictions on the dmesg output format as for
            :meth:`from_str` apply.
        """
        for i, line in enumerate(dmesg_out.splitlines() if dmesg_out else ''):
            if line.strip():
                try:
...
class DmesgCollector(CollectorBase):
    ...
    @memoized
    def _get_entries(self, dmesg_out: Optional[str], timestamp: Optional[timedelta],
    ...
    def _get_output(self) -> None:
        """
        get the dmesg collector output into _dmesg_out local variable
        """
    ...
    def reset(self) -> None:
        """
        Reset the collector's internal state (e.g., cached dmesg output).
        """
        self._dmesg_out = None

    def start(self) -> None:
        """
        Start collecting dmesg logs. If ``empty_buffer`` is true, clear them first.
    ...
    def stop(self) -> None:
        """
        Stop collecting logs and retrieve the latest dmesg output.
        """
        self._get_output()
    ...
    def get_data(self) -> CollectorOutput:
        """
        Write the dmesg output to :attr:`output_path` and return a :class:`CollectorOutput`.

        :raises RuntimeError: If :attr:`output_path` is unset.
        :return: A collector output referencing the saved dmesg file.
